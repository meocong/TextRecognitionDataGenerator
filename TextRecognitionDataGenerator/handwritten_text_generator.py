    ...
    def generate(cls, text):
        ...
        with tf.Session(config=config) as sess:
            saver = tf.train.import_meta_graph(
                'handwritten_model/model-29.meta')
            saver.restore(sess, 'handwritten_model/model-29')
            images = []
            for word in text.split(' '):
                _, _, _, stroke_data, coords = cls.__sample_text(
                ...
                for _, stroke in enumerate(cls.__split_strokes(cls.__cumsum(np.array(coords)))):
                ...
                image = Image.frombytes(
                    'RGB', canvas.get_width_height(), canvas.tostring_rgb()).convert('L')
