...
class ComputerTextGenerator(object):
    ...
    def generate(cls, text, font, text_color, height, text_mode, extend_bottom=0):
        ...
        if (text_mode == RANDOM_BIG_TEXT):
            ...
            image_font_big = ImageFont.truetype(
                font=font, size=int(height * random.uniform(1.0, 1.5)))
        ...
        if text_mode == TIGHT_TEXT:
            ...
            txt_img = Image.new(
                'L', (int(text_width * (f + 0.01)), text_height + extend_bottom), 255)
            txt_draw = ImageDraw.Draw(txt_img)

            offset_x = 0
            for c in text:
                char_w, _ = image_font.getsize(c)
                txt_draw.text((offset_x, start_pixel), u'{0}'.format(c),
                              fill=random.randint(
                                  1, 80) if text_color < 0 else text_color,
                              font=image_font)
            ...
            if (random.randint(0, 2) != 0 or len(text) < 6):
                ...
                txt_img = Image.new(
                    'L', (text_width1 + text_width2, max(text_height1, text_height2) + extend_bottom), 255)
                txt_draw = ImageDraw.Draw(txt_img)
                ...
                txt_draw.text((image_font.getsize(first_half)[0], start_pixel), u'{0}'.format(second_half),
                              fill=random.randint(
                                  1, 50) if text_color < 0 else text_color,
                              font=image_font_big)
            else:
            ...
            txt_img = Image.new(
                'L', (text_width, text_height + extend_bottom), 255)
            txt_draw = ImageDraw.Draw(txt_img)
            ...
            txt_img = Image.new(
                'L', (text_width + np.sum(addi), text_height + extend_bottom), 255)
            txt_draw = ImageDraw.Draw(txt_img)
