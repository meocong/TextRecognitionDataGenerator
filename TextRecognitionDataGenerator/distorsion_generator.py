...
class DistorsionGenerator(object):
    ...
    def apply_func_distorsion(cls, image, vertical, horizontal, max_offset, func):
        ...
        horizontal_offsets = [
            ...
            for i in range(
                img_arr.shape[0] + (
                    (max(vertical_offsets) -
                     min(min(vertical_offsets), 0)) if vertical else 0
                )
            )
        ]
        ...
        if vertical:
            ...
            for i, o in enumerate(vertical_offsets):
                ...
                new_img_arr[max_offset+o:column_height +
                            max_offset+o, column_pos, :] = img_arr[:, i, :]
        ...
        if horizontal:
            ...
            for i, o in enumerate(horizontal_offsets):
                if vertical:
                    ...
                    new_img_arr[i, max_offset+o:row_width +
                                max_offset+o, :] = img_arr[i, :, :]
