...
def sauvola_bin(img):
    img = np.array(img)
    bin = img > threshold_sauvola(img, window_size=15, k=0.3)
...
class FakeTextDataGenerator(object):
    ...
    def generate(cls, index, text, font, out_dir, height, extension, skewing_angle, random_skew, blur, random_blur, background_type, distorsion_type, distorsion_orientation, is_handwritten, name_format, text_color=-1, prefix = ""):
            # try:
                ...
                affine_type = 2 #random.randint(0,3)
                if affine_type == 0:
                ...
                # blur distortion
                blur_type = 2 #random.randint(0,3)
                ...
                if decision(0.3):
                    binary_im = Image.fromarray(sauvola_bin(final_image))
